...
'_____________________________________________ Medical Corpus ___________________________________________________'
...
mots_cibles = ['patient','traitement','maladie','jaune']

# chercher les 10 plus proches mots sans les scores de similarité de 4 mots 
prochesmots_patients_list,prochesmots_traitement_list,prochesmots_maladie_list,prochesmots_jaune_list = dix_mots_sans_score(model_CBOW_med)

# liste de tous les mots du corpus med
data_med_liste = model_CBOW_med.wv.key_to_index.keys()

# Dataframe contenant toutes les coordonnees en 2D de tous les mots du corpus med 
#df = coord_2D(model_CBOW_med, data_med_liste)

# Pour enregistrer le file la première fois histoire car il prend du temps, puis on aura juste besoin de le charger.
#with open('results_TSNE.pkl', 'wb') as file:    
    # A new file will be created
#    pickle.dump(df, file)

### Pour ouvrir le file une fois enregistré  ###
with open('results_TSNE.pkl', 'rb') as file:     
    # Call load method to deserialze
    df = pickle.load(file)

# Rechercher dans la variable df les coordonnees x,y des mots cibles et des mots les plus proches 
...
plt.title('Modèle Word2Vec : CBOW : CORPUS MÉDICAL')
plt.grid()
plt.legend()

# enregistrer le graphique
plt.savefig('model_CBOW_med.png')
plt.show()
...
# ouvrir le modèle et chercher les mots les plus proches : 
model_sg_med = Word2Vec.load("word2vec_sg_med.model")


# chercher les 10 plus proches mots sans les scores de similarité de 4 mots 
prochesmots_patients_list,prochesmots_traitement_list,prochesmots_maladie_list,prochesmots_jaune_list = dix_mots_sans_score(model_sg_med)

# liste de tous les mots du corpus med 
data_med_liste = model_sg_med.wv.key_to_index.keys()

# Dataframe contenant toutes les coordonnees en 2D de tous les mots du corpus med 
#df = coord_2D(model_sg_med, data_med_liste) # coordonnees en 2D de tous les mots du corpus med 

# Pour enregistrer le file la première fois histoire car il prend du temps, puis on aura juste besoin de le charger.
#with open('results_TSNE_sg.pkl', 'wb') as file:    
    # A new file will be created
#    pickle.dump(df, file)

### Pour ouvrir le file une fois enregistré  ###
with open('results_TSNE_sg.pkl', 'rb') as file:     
    # Call load method to deserialze
    df = pickle.load(file)
...
plt.title('Modèle Word2Vec : Skip Gram : CORPUS MÉDICAL')
plt.grid()
plt.legend()

plt.savefig('model_sg_med.png')
plt.show()
...
# ouvrir le modèle et chercher les mots les plus proches : 
model_ft_med = FastText.load("fasttext_CBOW_med.model")

# chercher les 10 plus proches mots sans les scores de similarité de 4 mots 
prochesmots_patients_list,prochesmots_traitement_list,prochesmots_maladie_list,prochesmots_jaune_list = dix_mots_sans_score(model_ft_med)

data_med_liste = model_ft_med.wv.key_to_index.keys() # liste de tous les mots du corpus med 
#df = coord_2D(model_ft_med, data_med_liste) # coordonnees en 2D de tous les mots du corpus med 

# Pour enregistrer le file la première fois histoire car il prend du temps, puis on aura juste besoin de le charger.
#with open('results_TSNE_ft.pkl', 'wb') as file:    
    # A new file will be created
#    pickle.dump(df, file)

### Pour ouvrir le file une fois enregistré  ###
with open('results_TSNE_ft.pkl', 'rb') as file:     
    # Call load method to deserialze
    df = pickle.load(file)
...
plt.title('Modèle FastText : CBOW : CORPUS MÉDICAL')
plt.grid()
plt.legend()

plt.savefig('model_ft_med.png')
